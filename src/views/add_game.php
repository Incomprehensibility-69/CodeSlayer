...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare(
        "INSERT INTO games (title, price, description, image_path, genre, release_date, stock, platform_id, tags, screenshots, reviews)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );
    $stmt->execute([$title, $price, $description, $image_path, $genre, $release_date, $stock, $platform_id, $tags, $screenshots, $reviews]);

    header("Location: games_list.php?message=Game added successfully");
    exit;
}
?>
    ...
    <main>
        ...
        <form name="gameForm" method="POST" class="game-form" onsubmit="return validateForm();" enctype="multipart/form-data">
            ...
            <label>Reviews (comma-separated):
                <textarea name="reviews" rows="3"></textarea>
            </label>

            <label>Cover Image:</label>
            <div id="drop-zone" style="border:2px dashed #ccc; padding:20px; text-align:center; border-radius:8px;">
               <p>Drag & drop an image here or click to select</p>
               <input type="file" name="image" id="image-input" accept="image/*" style="display:none;">
                 <img id="preview" src="#" alt="Preview" style="max-width:100%; margin-top:10px; display:none; border-radius:6px;">
            </div>
